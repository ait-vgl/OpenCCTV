...
public class OpenCCTVLiveStreaming {
    private final String TAG = "OpenCCTVLiveStreaming";
    ...
    public OpenCCTVLiveStreaming(Context context) {
        this.context = context;
    }

    public void fetchAvailableStreamsForUser() {
        FetchTask task = new FetchTask();
        task.execute(Operation.FETCH_ALL);
    }
    ...
    private class FetchTask extends AsyncTask<Operation, Void, Status> {
        ...
        @Override
        protected OpenCCTVLiveStreaming.Status doInBackground(Operation... params) {
            operation = params[0];
            ...
            try {
                switch (operation) {
                    case FETCH_ALL:
                        url = generateURL(getString(R.string.url_opencctv_live_streams));
                        break;

                    case FETCH_BY_ANALYTIC_INSTANCE_ID:
                        url = generateURL(getString(R.string.url_opencctv_live_streams_show));
                        break;
                ...
                return OpenCCTVLiveStreaming.Status.GENERIC_ERROR;
            }
            ...
            try {
                ...
                if (responseCode == 200) {
                    ...
                    if (response.contains(getString(R.string.common_forgot_password)))
                        return OpenCCTVLiveStreaming.Status.UNAUTHORIZED;

                    streamInfo = new JSONArray(response);

                    conn.disconnect();
                    return OpenCCTVLiveStreaming.Status.SUCCESS;
                }

            } catch (IOException e) {
                e.printStackTrace();
                return OpenCCTVLiveStreaming.Status.NETWORK_ERROR;
            } catch (JSONException e) {
                e.printStackTrace();
            }

            return OpenCCTVLiveStreaming.Status.UNKNOWN_ERROR;
        }

        @Override
        protected void onPostExecute(OpenCCTVLiveStreaming.Status status) {
            if (pDialog.isShowing())
                pDialog.dismiss();

            if (status == OpenCCTVLiveStreaming.Status.SUCCESS) {
        ...
        private void showSelectCameraDialog() {
            ...
            for (int i = 0; i < streamInfo.length(); i++) {
                try {
                    ...
                    adapter.add(stream.getString("camera_name"));
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setTitle(getString(R.string.dialog_select_camera));
