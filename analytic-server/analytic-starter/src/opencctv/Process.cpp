...
Process::Process()
{
	_pReadStream = NULL;
	_pid = 0;
	_iStatus = Process_Status_t::STOPPED;
}
...
bool Process::start(const std::string& sPathToExecutable, const std::string& sCommandLineParams)
{
    ...
    _iStatus = Process_Status_t::STARTING;
	if (!_pReadStream)
	{
	...
	} else
	{
		...
		if (_pid > 0)
		{
			_iStatus = Process_Status_t::STARTED;
			bRet = true;
		}
	}
	return bRet;
		...
		unsigned int iAnalyticInstanceId, std::string& sAnalyticPluginFilename, std::string& sAnalyticResultsDir)
{
	...
	if(_pid == 0)
	{
		...
		_iStatus = Process_Status_t::ERROR;
		_exit(-1);
	}
...
Process_Status_t Process::getStatus()
{
	int status;
	pid_t result = waitpid(_pid, &status, WNOHANG);
	if (result == -1) /* Error */
	...
	{
		_iStatus = Process_Status_t::STARTED;
	} else if (result == _pid) /* Child is finished */
	{
		_iStatus = Process_Status_t::STOPPED;
	}
