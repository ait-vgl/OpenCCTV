...
void TcpMq::write(const string& message)
{
	...
	if(!isSent)
	{
		cerr << "TcpMq: Zmq Error: " << "Unable to send message to Zmq server." << endl;
		// TODO: throw application exception
	}
}
...
string TcpMq::read()
{
	...
	if(!isSent)
	{
		cerr << "TcpMq: Zmq Error: " << "Unable to read message from Zmq server." << endl;
		// TODO: throw application exception
	}
...
{
	(*socketPtr).close();
	delete socketPtr; socketPtr = 0;
	(*contextPtr).close();
	delete contextPtr; contextPtr = 0;
}
} /* namespace mq */
